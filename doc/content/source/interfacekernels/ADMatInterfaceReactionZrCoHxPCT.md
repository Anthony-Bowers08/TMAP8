...
For overall PCT capabilities [/ZrCoHx_PCT_Overall.i] tests the entire PCT modelling curves in TMAP8. The model follows the same structure as [/ZrCoHx_PCT.i], but has an arbitrarily high diffusion value to acheive steady-state quickly. The model also includes a linear pressure increase to cover the entire PCT curve.
